...
export default function Training() {
  ...
  return (
    <section id="training" className="bg-frost py-16 sm:py-20 lg:py-32">
      <div ref={ref} className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <h2 data-animate className="font-display text-[clamp(2.5rem,7vw,5.5rem)] tracking-[0.02em] text-ink leading-[1.05] mb-10 lg:mb-12 opacity-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[60%_40%] gap-10 lg:gap-12">
          <div data-stagger className="min-w-0">
            <div className="overflow-x-auto -mx-4 sm:-mx-6 lg:mx-0 px-4 sm:px-6 lg:px-0">
              <div className="min-w-[600px] lg:min-w-0 rounded-lg border border-[rgba(26,28,35,0.08)] overflow-hidden">
                  ...
                  <tbody>
                    {schedule.map((row, i) => (
                      <tr
                        key={i}
                        data-stagger-item
                        className={`opacity-0 ${i % 2 === 0 ? 'bg-paper' : 'bg-frost'}`}
                      >
                        <td className="px-4 sm:px-5 py-3.5 font-medium text-ink whitespace-nowrap">{row.day}</td>
                        <td className="px-4 sm:px-5 py-3.5 text-ink whitespace-nowrap">{row.time}</td>
                        <td className="px-4 sm:px-5 py-3.5 text-ink">{row.group}</td>
                        <td className="px-4 sm:px-5 py-3.5 text-slate whitespace-nowrap">{row.location}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div data-stagger className="space-y-6">
            {locations.map((loc, i) => (
              <div
                key={i}
                data-stagger-item
                className="bg-paper rounded-lg border border-[rgba(26,28,35,0.08)] p-5 sm:p-6 opacity-0 hover:-translate-y-1 hover:shadow-lg transition-all duration-300"
              >
                {loc.label && (
                  <span className="inline-block text-xs font-medium uppercase tracking-[0.05em] bg-[rgba(232,80,44,0.15)] text-ember rounded-full px-3 py-1 mb-3">
                    {loc.label}
                  </span>
                )}
                <h3 className="text-lg font-semibold text-ink tracking-tight mb-1">{loc.title}</h3>
                <p className="text-sm text-ink mb-1">{loc.detail}</p>
