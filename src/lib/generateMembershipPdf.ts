...
function checkbox(doc: jsPDF, x: number, y: number, round: boolean) {
  doc.setDrawColor(0)
...
export function buildMembershipPdf(): jsPDF {
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(11)
  doc.text('Mitgliedschaft und Kosten (bitte 1 Option ankreuzen)', MARGIN, y)
  y += 6
  ...
  membershipTiers.forEach((tier) => {
    ...
    doc.text(tier.label, MARGIN + 6, y)
    doc.setFont('helvetica', 'bold')
    doc.text(`€ ${tier.price} / Jahr`, PAGE_WIDTH - MARGIN, y, { align: 'right' })
    y += 5.5
  })
  ...
  doc.setFontSize(8)
  const oebv = doc.splitTextToSize(
  ...
  y += 1
  checkbox(doc, MARGIN + 3, y, false)
  doc.setFontSize(9)
  const junior = doc.splitTextToSize(
    `Ich melde mein Kind zusätzlich zum Nachwuchstraining an. Der Selbstkostenbetrag dafür beträgt pro Schulsemester € ${juniorTraining.price} und wird jeweils zum Semesterbeginn vorgeschrieben.`,
    CONTENT_WIDTH - 12
  )
  doc.text(junior, MARGIN + 9, y)
  y += junior.length * 4 + 4
