...
export default function Beitrittserklaerung() {
  ...
  return (
    <div className="min-h-screen bg-[#d9d9d9] print:bg-white">
      ...
      <div className="print:hidden sticky top-0 z-10 bg-card border-b border-theme">
        <div className="max-w-[900px] mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          <Link to="/anmeldung" className="inline-flex items-center gap-2 text-sm text-muted hover:text-primary transition-colors">
            <ArrowLeft className="w-4 h-4" />
            {t.registration.previewBack}
          </Link>
          <div className="flex items-center gap-3">
            <button
              onClick={() => window.print()}
              className="inline-flex items-center gap-2 border border-theme text-primary text-sm font-semibold rounded-full px-5 py-2.5 hover:bg-card-alt transition-all duration-200"
            >
              <Printer className="w-4 h-4" />
              {t.registration.printButton}
            </button>
            <button
              onClick={() => generateMembershipPdf()}
              className="inline-flex items-center gap-2 bg-accent-gradient text-white text-sm font-semibold rounded-full px-6 py-2.5 hover:-translate-y-0.5 transition-all duration-200"
            >
              <FileDown className="w-4 h-4" />
              {t.registration.downloadPdfButton}
            </button>
          </div>
        </div>
      </div>

      {/* Printable A4 sheet — kept as a fixed white/black legal document regardless of site theme */}
      <div className="max-w-[900px] mx-auto py-8 px-4 sm:px-6 print:py-0 print:px-0 print:max-w-none">
        <div className="bg-white text-black rounded-lg print:rounded-none shadow-xl print:shadow-none p-8 sm:p-10 print:p-[14mm] text-[13px] leading-snug">
          ...
          <div className="flex items-start justify-between gap-6 border-b-2 border-black pb-4 mb-4">
            ...
            <div className="text-right shrink-0">
              <div className="font-display text-2xl leading-none">
                BSC 70 <span className="text-accent">LINZ</span>
              </div>
          ...
          {/* Membership & fees */}
          <div className="mb-4">
            <h2 className="font-semibold text-[14px] mb-2">Mitgliedschaft und Kosten (bitte zutreffende Option(en) ankreuzen)</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1.5">
              {fees.map((fee) => (
                <div key={fee.label} className="flex items-baseline gap-2">
                  <span className="inline-block w-3.5 h-3.5 rounded-full border border-black shrink-0" />
                  <span className="flex-1">{fee.label}</span>
                  <span className="font-semibold whitespace-nowrap">{fee.price}</span>
                </div>
              ))}
            </div>
            <p className="text-[10.5px] mt-2 italic">{note}</p>
          </div>
        ...
        <p className="print:hidden text-center text-muted text-xs mt-6">
          Bitte ausgefüllt und unterschrieben per E-Mail an{' '}
          <a href={`mailto:${clubInfo.registrationEmail}`} className="text-accent hover:underline">{clubInfo.registrationEmail}</a>{' '}
